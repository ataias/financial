...
module Interest =
    ...
    type EasyInvestTable = CsvProvider<"../data/easyinvest-2019-no-sep.csv", ";", PreferOptionals=true, Culture="pt-BR">

    let rec public interest rate amount delta n =
        match n with
        | 0 -> amount
        | _ -> interest rate (amount * (1. + rate) + delta) delta (n - 1)
    ...
    let public simulate nYears =
        let rates =
            [ for i in 1 .. 15 -> 0.1 * (float i) / 100.0 ]

        let n = rates.Length
        let initialAmount = 120000.0

        let delta =
            [ for i in 10 .. 50 -> float (i * 100) ]
    ...
    let public printArray (arr: string [,]): unit =
        for line = 0 to Array2D.length1 arr - 1 do
            if line = 1 then printfn "|-"
            for column = 0 to Array2D.length2 arr - 1 do
                // printfn "(%d, %d)" line column
                printf "| %s " arr.[line, column]
            printfn "|"

    let public createTable title (data: float [,]) (top: Header list)
        (left: Header list): string [,] =
        let genElement i j =
            match (i, j) with
            | 0, j when j > 0 ->
                let h = top.[j - 1]
    ...
    let public read (broker: Broker) (uri: string): InvestmentInfo [] =
        ...
        | EasyInvest ->
            ...
            Seq.map convert data.Rows |> Array.ofSeq
